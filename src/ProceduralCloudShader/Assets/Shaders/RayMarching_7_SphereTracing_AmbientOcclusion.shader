Shader "Custom/RayMarching/SphereTracing Ambient Occlusion"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float sceneSDF(float3 position) {
				...
				float dCylinder1 = cylinderSDF(position, fixed3(3,1,1), fixed3(-2,1,1), 0.9, fixed3( 0, 0.1,-1));
				float dCylinder2 = cylinderSDF(position, fixed3(1,3,1), fixed3(1,-1,1), 0.9, fixed3(-1, 1.1,-1));
				float dCylinder3 = cylinderSDF(position, fixed3(1,1,3), fixed3(1,1,-2), 0.9, fixed3(-1, 0.1, 0));
