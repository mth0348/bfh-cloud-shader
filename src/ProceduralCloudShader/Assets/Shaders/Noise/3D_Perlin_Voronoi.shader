Shader "Custom/Noise/3D/Perlin_Voronoi"
{
    Properties
    {
        [Header(Perlin)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float3 co1 = float3(i.worldPos.x, i.worldPos.y, i.worldPos.z) *_PerlinScale + _PerlinOffset*t;
                float3 co2 = float3(i.worldPos.x, i.worldPos.y, i.worldPos.z) *_VoronoiScale + _VoronoiOffset*t;

                float f1 = getColorPerlin(co1, _PerlinOctaves, _PerlinPersistance);
                float f2 = getColorVoronoi(co2, _VoronoiOctaves, _VoronoiPersistance);
                
                fixed3 col = fixed3(0,0,0);

                if (_Mask == 0) { // Perlin Master
                    f2 = 0.5 + 0.5 * f2;
                    f1 = 0.5 + 0.5 * f1;
                    
                    col += 1 - smoothstep(_VoronoiMin, _VoronoiMax, f2 - _VoronoiBoost);
                    col *= smoothstep(_PerlinMin, _PerlinMax, f1 + _PerlinBoost);
                }
                else if (_Mask == 1) { // Voronoi Master
                    f1 = 0.5 + 0.5 * f1;
