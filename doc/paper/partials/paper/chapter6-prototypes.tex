...
$ d_{step} = \frac{d_{box}}{N}$, where $d_{box}$ is equal to the total distance the ray travels while inside the box. To determine $d_{box}$, an \gls{aabb} (AABB) intersection test has to be done with the container cube and the ray.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{unity captures/final doc/final_dusk_small.PNG}
...
Before talking about measuring the realism of the cloud shader, it seems important to objectively identify the capabilities of it first.
...
\\
In the following subsection, each rendered image is compared to a real-life photograf, putting them into a objectively comparable state to reality.
...
\begin{figure}[H]
    ...
    \captionof{figure}{Camparison: photografic reference \protect\cite{img:campare:altocumulus} versus the rendered image.}
...
\begin{figure}[H]
    ...
    \captionof{figure}{Camparison: photografic reference \protect\cite{img:campare:altocumulus:night} versus the rendered image (at night).}
...
Unfortunately, the difference of details and density is fairly discernable and the desired appearance is not fully achieved.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{comparisons/compare_altocumulus_sunset.jpg}
    \captionof{figure}{Camparison: photografic reference \protect\cite{img:campare:altocumulus:sunset} versus the rendered image (at sunset).}
...
\begin{figure}[H]
    ...
    \captionof{figure}{Camparison: photografic reference \protect\cite{img:campare:altocumulus:stormy} versus the rendered image (stormy).}
    \label{img:camparisons:altocumulus:stormy}
\end{figure}

\clearpage
\subsubsection{Convolutional Neural Network}
Given there is a \gls{cnn} (CNN) that is able to classify images of the sky, the weather or clouds into descriptive labels or even genera of cloud formations, then one could just seed those rendered images into the CNN and verify wether the results are truthful.
