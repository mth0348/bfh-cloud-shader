...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[scale=0.7]
        ...
        \node[red] (p1) at (-1.5,1) {};
        \node[red] (p2) at (2.5,3) {\ding{53}};
        \draw[red,shorten >=-8pt] (p1) node[below right]{camera ray} -- (p2);
        \draw[-{Latex[length=2mm]},shorten <=-6pt,cyan] (p2) node[below right]{stepped forward} -- (4.75,4.125);
...
\noindent
The ray-surface intersection is not directly calculated because it is too complex for volumes like clouds, which is why the surface distance is approximated instead.
...
\noindent
As clearly visible, the traditional \gls{raymarching} ends up with a more accurate result and the amount of steps per ray could be relatively lower, ultimately saving performance. 
\\
