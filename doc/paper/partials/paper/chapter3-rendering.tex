...
In \gls{raymarching}, adding shadows to the scene is not difficult. As defined in \autoref{lst:shader:raymarch:spheretracing}, the \lstinline[language=HLSL]{raymarch()} function moves along the given ray and returns the distance to the nearest intersection of ray and volume.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[scale=1.2]
        ...
        % icons
        \node[icon] (cam) at (0,2) {\faVideoCamera};
        \node at (0,2.6) {camera};
